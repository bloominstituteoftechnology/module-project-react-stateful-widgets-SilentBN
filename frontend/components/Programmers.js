...
export default function Programmers() {
  ...
  const [selectProgrammer, setSelectedProgrammer] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    return programmers.find((pr) => pr.id === selectProgrammer).name;
  };

  const style = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: selectProgrammer ? "gold" : "royalblue", // 🤔 color turns to gold, when celebrating
  };
